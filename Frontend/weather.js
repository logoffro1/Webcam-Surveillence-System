...
GLoc = {
    ...
    geoSuccess: function(position) {
        ...
        g.startPos = position;
        g.searchQuery = 'http://api.weatherapi.com/v1/current.json?key=467d5d2f62c441e7a4390108232104&q=Haarlem&aqi=no';
...
WeatherInfo = {
    ...
    setWeatherData: function(data) {
        ...
        w.location.text('Haarlem, ' + data.location.country);
        w.humidity.text(data.current.humidity);
        w.weatherDescription.text(data.current.condition.text);
        w.tempNumber = data.current.temp_c;
        w.windSpeed = data.current.wind_kph;
        w.windDegree = data.current.wind_degree;
...
$(function() {
    ...
    GLoc.geoSuccess("52.3886245,4.6116576");
});
